...
import { encodeMessage, constants } from "./common/index";
...
export class BulkPull extends EventEmitter {
  ...
  constructor(host: any, port: number, experimental = false) {
    super();
  ...
  request({ start, end = Buffer.alloc(32), count, ascending = false }) {
    ...
    const message = Buffer.alloc(64 + (hasCount ? 8 : 0));
    message.set(start);
    message.set(end, 32);
    if (hasCount) {
    ...
    this.socket.write(
      encodeMessage({
        ...
        extensions: (hasCount ? 1 : ascending && 2),
      })
    );
  }
